...
namespace _3._Scripts.UI.Panels
{
    ...
    {
        [Header("Sections")] [SerializeField] private List<LevelBarSection> levelBarSections = new();
        ...
        [Header("Final")] [SerializeField] private RectTransform finalContainer;
        [SerializeField] private Image finalIcon;
        private readonly List<Image> sections = new();
        private Tween currentStageTween;
        protected override void OnOpen()
        {
            ...
            SetSize();
            SetInfo(contract);
            currentStageTween = sections[LevelHandler.CurrentStageIndex].DOColor(current, 0.5f).SetLoops(-1, LoopType.Yoyo)
                .SetLink(sections[LevelHandler.CurrentStageIndex].gameObject);
            for (var i = 0; i < LevelHandler.CurrentStageIndex; i++)
            {
        ...
        {
            currentStageTween.Pause();
            currentStageTween.Kill();
            currentStageTween = null;
        }
        ...
        {
            foreach (var level in contract.Stages.Where(l => l.Type != StageType.Boss))
            {
